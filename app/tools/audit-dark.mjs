...
const FONDS = { bg: hsl2rgb(220, 6, 7), panel: hsl2rgb(220, 6, 12), panel2: hsl2rgb(220, 6, 16) };
...
while ((m = re.exec(css))) {
  ...
  const rgb = parse(val);
  if (!rgb) continue;
  const r = ratio(rgb, FONDS.panel);
  if (r < 4.5) problemes.push({ sel: sels[0], val, r: r.toFixed(2) });
}
problemes.sort((a, b) => a.r - b.r);
console.log(`Couleurs de texte en dur illisibles sur le panneau sombre : ${problemes.length}`);
for (const p of problemes.slice(0, 25))
  console.log(`  ${p.r.padStart(5)}:1  ${p.val.padEnd(26)} ${p.sel.slice(0, 58)}`);
