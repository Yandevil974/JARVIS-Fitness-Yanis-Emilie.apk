...
export function interpretCommand(p, text) {
  ...
  const performance =
    q.match(
      /(\d+(?:[.,]\d+)?)\s*(?:kg|kilos?)[\s\S]*?(\d+)\s*(?:reps?|repetitions?)/,
    ) || q.match(/(\d+(?:[.,]\d+)?)\s*[x×]\s*(\d+)/);
  if (performance) {
  ...
  return {
    text: "Je n’ai pas reconnu cette demande. Je ne vais pas inventer une réponse. Essayez « Je n’ai que 30 minutes », « Remplace cet exercice », « Je suis fatigué » ou « 4 séances par semaine pendant 12 semaines ».",
  };
}
