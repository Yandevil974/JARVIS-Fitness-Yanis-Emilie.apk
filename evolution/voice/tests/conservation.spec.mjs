...
async function content(page){return page.locator('main').evaluate(main=>{
  const nodes=[...main.querySelectorAll('.jv-panel,.chat-composer')],before=nodes.map(n=>n.style.display);
...
for(const id of ['elite','emilie'])for(const width of [390,1440])test(`Step 2 retains complete screens and reminder board: ${id}, ${width}px`,async({page,browser})=>{
  ...
  await page.goto('/');await reference.goto(process.env.STEP1_URL||'http://127.0.0.1:5176');
