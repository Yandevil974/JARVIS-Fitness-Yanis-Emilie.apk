...
import { integrate as voice } from "../../appointments/build.mjs";
...
test("stage 5 conserves every previous declaration except the dashboard/training panels", () => {
  ...
  assert.deepEqual(changed.sort(), ["V5", "t2"]);
  assert.ok(next.has("JarvisSpokesperson"));
  assert.ok(next.has("JarvisVoice"));
  assert.ok(next.has("JarvisFollowUp"));
});
test("all complete screens, local team metadata, timers and store are byte-identical to stage 4", () => {
