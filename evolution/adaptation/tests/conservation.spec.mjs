...
async function content(page) {
  return page.locator("main").evaluate((main) => {
    const nodes = [...main.querySelectorAll(".je-adaptation")],
...
for (const id of ["elite", "emilie"])
  for (const width of [390, 1440])
    test(`Step 5 retains complete screens and stages 1–4: ${id}, ${width}px`, async ({
      page,
      browser,
    }) => {
      ...
      await page.goto("/");
      await reference.goto(process.env.STEP4_URL || "http://127.0.0.1:5177");
