...
async function content(page) {
  return page.locator("main").evaluate((main) => {
    const nodes = [
        ...main.querySelectorAll(".jd-owned,.je-adaptation > small"),
      ],
    ...
    const text = main.innerText
      ...
      .replace(/V 1\.[012]\.[06]/g, "V RELEASE")
      .replace(/\s+/g, " ")
      .trim();
...
for (const id of ["elite", "emilie"])
  for (const width of [390, 1440])
    test(`Step 6 retains complete screens and stages 1–5: ${id}, ${width}px`, async ({
      page,
      browser,
    }) => {
      ...
      await page.goto("/");
      await reference.goto(process.env.STEP5_URL || "http://127.0.0.1:5178");
      ...
      for (const screen of screens) {
        ...
        for (let i = 0; i < tabs.length; i++) {
          ...
          expect(await content(page), `${screen}/${tabs[i]}`).toBe(
            await content(reference),
          );
        }
      }
