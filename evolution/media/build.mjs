...
entry(i){return i&&i.id&&(JarvisMediaMap.exercises[i.id]||JarvisMediaMap.stretchOverrides[i.id])||null},
...
export function integrate(source) {
  ...
  source = replaceOnce(
    ...
    'function s5(i,o,ctx){const n=i?Kh(i):null;if(n)return{path:n.path,name:n.name,exact:n.level==="exact",level:n.level,note:n.note||null};if(i){const so=JarvisMediaMap.stretchOverrides[i.id]||null;if(i.img)return{path:i.img,name:i.name,exact:!so||so.level==="exact",level:so?so.level:"exact",note:so?so.note:null};if(so&&!so.media)return{missing:!0,note:so.note||null,name:i.name}}if(o)return JarvisMedia.movement(o,ctx);return null}const Wf={exact:"Illustration humaine · source",variante:"Même mouvement, détail différent · voir la note",famille:"Même mouvement, détail différent · voir la note"};',
  );
