...
test('audit is pinned to the actually delivered APK, and is not a new release',()=>{
 ...
 assert.equal(findings.releaseReady,false); // no signed APK produced by this repository state
 assert.ok(['open','delivered-in-mapping'].includes(findings.status));
});
...
test('findings reference real catalog entries and cover every requested category',()=>{
 ...
 assert.deepEqual(new Set(findings.findings.map(f=>f.scope)),new Set(['musculation','echauffement','piscine','tabata','etirements']));
 ...
 for (const f of findings.findings) {
  ...
  assert.ok(['open','corrected','explicit-absence','partial'].includes(f.status),f.id);
  if(f.status!=='open') assert.ok(f.resolution&&f.resolution.length>20,f.id+' needs a reviewable resolution');
 }
});
