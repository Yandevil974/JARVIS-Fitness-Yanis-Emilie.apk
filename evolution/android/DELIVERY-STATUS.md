...
- Tests : `evolution/media/tests/media-inventory.test.mjs` mesure **l'APK**, reproduit le défaut sur la 1.4.7 et exige la présence et l'empreinte des animations livrées dans la 1.4.8.
- **Version de travail en cours** (elle sera remplacée par la version complète demandée le 24 septembre) : 30 animations du **Tabata au sol** + 5 noms équivalents, soit **35 des 38 noms** du générateur au sol ; APK de contrôle `f39cc05a…`, 27 210 438 octets, 37 entrées média. Résolution **uniquement** en contexte Tabata au sol (jamais piscine, étirement, échauffement, repos) ; les 3 noms restants (High knees, Repos actif, Respiration profonde) sont listés et mesurés par le test, ils gardent l'affichage actuel.
