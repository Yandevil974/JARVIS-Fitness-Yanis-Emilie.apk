...
## 1.4.8 — 24 septembre 2026 (version complète : tous les médias manquants embarqués)

- `downloads/Yanis-Fitness-Evolution-1.4.8.apk` — **28 035 594 octets**, SHA-256 `fee667192291dd8adc93593bb395b6193e82849e11a4d5db94bf9218c8b99e9f`, versionName 1.4.8 / versionCode 19.
- Identité durable `150e3846d867aae1d08694d0d5d2b53e404f77ca635edb88055618b6d769d37b` : installation directe par-dessus la 1.4.6, la 1.4.7 ou toute version 1.4.2 et suivante, sans désinstallation. Les versions 1.4.3, 1.4.4, 1.4.5, 1.4.6 et 1.4.7 partagent cette signature.
- Web embarqué `ce7c4b897a1d2413c91ccb2073a318e91a7771c825f86c09abd8de7304bc4dc6` (326 fichiers web), 9/9 DEX identiques octet pour octet, signature v2 + v3 vérifiée, alignement vérifié, un seul signataire.
- Le correctif de la 1.4.7 est appliqué **et** les médias qui manquaient à la 1.4.7 sont embarqués : `gainage-vertical.gif`, `mobilite-epaules.gif`, `mobilite-hanches-chevelles.gif`, `ciseaux-au-bord.gif`, `talons-fesses.gif`. La construction refuse de produire un APK si un chemin `/media`, `/thumbs` ou `/team` référencé par le script livré est absent de l'archive.
- **Tous les visuels humains manquants sont produits et embarqués (54 fichiers média ajoutés)** : 33 mouvements du Tabata au sol (+ 5 noms équivalents de son générateur), 4 étirements dont le dessin montrait un autre mouvement, 10 variantes d'alias résolues par identifiant exact, 2 animations de récupération piscine. Aucun n'affiche plus un dessin d'un autre exercice, un vélo, un elliptique ou la photo générique.
- **Mesure de la récupération piscine** (six protocoles, 420 étapes) : 117 « Repos » + 18 « … — en place » ne résolvaient aucun média aquatique ; elles affichent désormais une animation produite, uniquement en contexte piscine. Les 420 étapes résolvent un média aquatique : **zéro vélo, zéro elliptique, zéro photo générique**.
- **Mesure du script livre lui-meme** (`evolution/media/tests/delivered-bundle.test.mjs`, ajoute le 24 septembre) : le JavaScript extrait de cet APK est evalue tel quel et resout les 420 etapes piscine (table exacte : 117 `pool-repos.gif`, 18 `pool-en-place.gif`, zero chemin `cardio-`/`elliptique`/photo), les 38 noms du Tabata au sol, les 10 variantes d'alias et les 4 etirements. Ce n'est plus seulement la source qui est testee, c'est le fichier telecharge.
- **Construction reproductible** : relancer `python3 evolution/android/build-media-148.py --real` sur les sources commitees reproduit **exactement** le meme SHA-256 (`fee66719…`, 28 035 594 octets).
- Tests : **64/64 Node**, dont `media-inventory.test.mjs` qui mesure **l'APK livré** (présence et empreinte de chaque média), `pool-candidate.test.mjs` (420 étapes, contextes, récupération), `alias-visuals.test.mjs`, `tabata-land-media.test.mjs`, `stretch-media.test.mjs`.
- Constats mis à jour, **aucun fermé silencieusement** : les groupes concernés restent **ouverts** jusqu'à l'essai sur le téléphone de l'utilisateur.
- **Défaut mesuré après livraison (24 septembre 2026, constat ouvert `library-thumbnails-missing-for-produced-variants`)** : dans la **bibliothèque d'exercices**, la vignette d'une carte est calculée depuis le GIF servi (`/thumbs/<nom>.webp`, jamais écrite en toutes lettres dans le script, donc invisible pour le garde-fou ci-dessus). Les **10 variantes produites** n'ont pas de vignette dans l'APK ⇒ **10 cartes de la bibliothèque affichent une image cassée**. La démonstration (GIF entier) et le chrono ne sont pas touchés : la 1.4.8 reste la version à installer. Correction prévue dans la **prochaine version complète** (vignette pour chaque GIF servi + construction qui refuse un APK s'il en manque une) ; reproduit par `evolution/media/tests/restyle-inventory.test.mjs`.

## Chantier suivant — refonte de tous les GIF d'après la photo de l'utilisateur (ouvert le 24 septembre 2026)

- Demande : « Refaire tous les gif animés en fonction de l'image que je te joins en photo […] un modèle féminin pour Emilie. Faire un visage ». **La photo n'a pas été reçue** : aucune image générée, aucun APK construit.
- Périmètre mesuré sur l'APK livré (`evolution/media/review/RESTYLE-INVENTAIRE.json`) : 144 GIF, dont 139 affichés, + 10 variantes du lot 2 + 3 à produire = **152 mouvements** ; 95 sont vus par Émilie.
- Modèle féminin : le profil actif (`elite` / `emilie`) et une fonction d'adresse unique (`bt`) pour les 20 images de l'application permettent de servir la version de chaque profil sans toucher aux données ; médias livrés jamais écrasés, nouveaux fichiers ajoutés.
- Livraison : une seule version complète à la fin, signée `150e3846…` (clé de récupération à recoller : `/tmp/rk.txt` effacé). Tests actuels : **66/66**.
