...
import { integrate as voice } from "../../spokesperson/build.mjs";
...
test("stage 4 conserves every previous declaration except the dashboard/training entry points and modal route", () => {
  ...
  assert.deepEqual(changed.sort(), ["Q5", "V5", "t2"]);
  assert.ok(next.has("JarvisSpokesperson"));
  assert.ok(next.has("JarvisVoice"));
  assert.ok(next.has("JarvisFollowUp"));
});
test("all complete screens, local team metadata, timers and store are byte-identical to stage 3", () => {
