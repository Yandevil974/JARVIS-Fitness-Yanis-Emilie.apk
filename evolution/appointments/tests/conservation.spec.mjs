...
async function content(page) {
  return page.locator("main").evaluate((main) => {
    const nodes = [...main.querySelectorAll(".ja-board")],
...
for (const id of ["elite", "emilie"])
  for (const width of [390, 1440])
    test(`Step 4 retains complete screens and stages 1–3: ${id}, ${width}px`, async ({
      page,
      browser,
    }) => {
      ...
      await page.goto("/");
      await reference.goto(process.env.STEP3_URL || "http://127.0.0.1:5175");
