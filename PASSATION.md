...
### Lots 1–3 intégrés (22/09, après « Poursuis » ×2) — réversibles

- **17 dessins** dans les styles existants (GIF 2 positions 650 ms ; fiches d’étirement fixes) : 14 mouvements Tabata au sol (+ 4 variantes annotées), mobilité hanches & chevilles, activation scapulaire, mollet en escalier, adduction de la hanche debout. Fichiers dans `evolution/media/assets/`, noms dans `VALIDATED_PROPOSALS` (`review/compose-mapping.py`) ou entrées `warmup`/`stretches` ; planches `review/proposals-lot1..3.jpg`. **L’utilisateur n’a pas validé image par image** (réponses « Poursuis » + « c’est long ») : si un dessin ne convient pas, retirer son nom / remettre `N(...)`, relancer la chaîne — la carte d’absence revient.
- Chaîne relancée : `compose-mapping.py` → `build.mjs` (pin `87d7fea0…`) → 11 tests Node → **132 contrôles DOM** → `build-media.py --unsigned-candidate` (18 entrées média, 5 tests Python OK).
- **Reste** : 26 exercices de musculation avec matériel (prochain lot de dessins), « Échauffement progressif » (échauffement libre, absence assumée), « Étirements au bord » (piscine, partiel). Et toujours : **signature 1.5.0 bloquée** sans la sauvegarde privée.
- Environnement réinitialisé deux fois le 22/09 : tout est sur la branche distante ; remise en état = `git merge --ff-only origin/arena/01a0c424-…`, `npm install --no-save acorn` dans `JARVIS-Fitness-Source`, `pip install --target .cache/image-tools pillow`, `npm install happy-dom` dans `.cache/dom-tools`, extraction du web 1.4.0 vers `.cache/media-audit/web-1.4.0/`.
