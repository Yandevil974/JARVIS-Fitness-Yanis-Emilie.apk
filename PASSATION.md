...
## Dernière avancée — APK 1.4.1 signé, vérifié et publié (23 septembre 2026)

**Ce n'est plus un candidat : l'application corrigée est construite, signée et poussée.** L'utilisateur a explicitement autorisé une nouvelle clé le 23 septembre 2026 (« si faut en construire une autre avec une nouvelle clé tu as mon autorisation »). Les deux certificats existants (`7d6f9c8f…` de la 1.4.0 et `4d4fbd84…` historique) ne sont **ni régénérés ni remplacés** ; la nouvelle identité est distincte et son empreinte publique est `d3ca5a27cf9b97f15404b1d1dad5c35abf5ec39e4917d0d30e89078a71782a3d`.

[Télécharger la 1.4.1](https://raw.githubusercontent.com/Yandevil974/JARVIS-Fitness-Yanis-Emilie.apk/f69f469/downloads/Yanis-Fitness-Evolution-1.4.1.apk) · [consignes](https://raw.githubusercontent.com/Yandevil974/JARVIS-Fitness-Yanis-Emilie.apk/f69f469/downloads/INSTALLATION-1.4.1.md) · [empreinte](https://raw.githubusercontent.com/Yandevil974/JARVIS-Fitness-Yanis-Emilie.apk/f69f469/downloads/Yanis-Fitness-Evolution-1.4.1.apk.sha256) · [inventaire](https://raw.githubusercontent.com/Yandevil974/JARVIS-Fitness-Yanis-Emilie.apk/f69f469/downloads/Yanis-Fitness-Evolution-1.4.1.fidelity.json)

- **APK livré** : `downloads/Yanis-Fitness-Evolution-1.4.1.apk`, **24 909 281 octets**, SHA-256 `c8dff99767923e1a452acffd0484d3e19cc989bacad212ee3cbb47bef01a668d`, `versionName` 1.4.1 / `versionCode` 12, paquet `app.yanis.fitness.evolution.home`. Empreinte de contrôle dans `downloads/Yanis-Fitness-Evolution-1.4.1.apk.sha256`, inventaire complet dans `downloads/Yanis-Fitness-Evolution-1.4.1.fidelity.json`, consignes dans `downloads/INSTALLATION-1.4.1.md`.
- **Fidélité à la 1.4.0 prouvée, pas affirmée** : sur les 272 fichiers web, **271 sont inchangés** ; les **9 bibliothèques natives `.dex` sur 9 sont identiques octet pour octet** ; seules deux entrées du ZIP changent — `AndroidManifest.xml` (version 1.4.1, code 12) et `assets/public/assets/index-CBCies4k.js`, qui est **exactement** le bundle candidat revu `b74853bc…`. Aucun exercice, aucune prescription, aucune image, aucun réglage, aucun historique n'est modifié.
- **Signature vérifiée indépendamment** : `apksigner verify --print-certs` → *Verifies*, un seul signataire, **schémas v2 et v3 vrais**, v1/v4 faux comme voulu, alignement vérifié. Le vérificateur est le `apksigner.jar` reconstruit localement, d'empreinte `ef494179…` = celle épinglée dans `release-home.json`.
- **Sauvegarde privée remise à l'utilisateur** : `Yanis-Fitness-Evolution-1.4.1-SAUVEGARDE-PRIVEE.zip` (`.p12`, mot de passe, empreinte, mode d'emploi) est **ignoré par Git** et vit aussi dans `.private/yanis-fitness-evolution-media/` (droits 700/600). Seule l'empreinte publique est versionnée (`evolution/android/identity-media-141.json`). **Sans ce ZIP, aucune mise à jour future de cette application n'est possible.**
- **Installation** : même identifiant d'application qu'en 1.4.0, signature différente ⇒ Android refuse la mise à jour par-dessus. Ordre obligatoire : **exporter le JSON de sauvegarde dans la 1.4.0 → désinstaller la 1.4.0 → installer la 1.4.1 → restaurer le JSON.** Aucune donnée n'est perdue si l'export est fait d'abord.
- **Ce que la 1.4.1 corrige** : les étapes aquatiques (marche aquatique, nage douce, fractionné nagé) affichent le guide aquatique **dans l'aperçu et dans le chrono**, y compris quand le bloc piscine suit une séance de musculation — les deux réécritures de code livré (`G4` aperçu, `Mg` chrono) décrites ci-dessous sont **dans l'APK**. Jamais de vélo ni d'elliptique pour une récupération en piscine.
- **Tests au moment de la livraison** : **38 tests Node PASS** (dont une nouvelle traçabilité : les 94 dessins livrés sont relus au moins une fois, et « relu » n'entraîne **jamais** « validé ») ; **26 parcours navigateur PASS (3,5 min)** et **8 accueil / 11 rubriques PASS (57,9 s)** exécutés sur ce même paquet web. Aucun test sur téléphone réel n'est possible dans cet environnement : c'est une limite, pas une preuve.
- **Dessins** : 94 GIF sur 94 relus au moins une fois (46 longs, 13 courts, 5 cibles d'alias, 30 lors de la dernière passe : 60 images). Verdicts de cette passe : 19 conformes, 5 variantes à confirmer, 5 écarts partiels, 1 écart gardé ouvert. **Aucun dessin n'est déclaré définitivement accepté** (`finalAccepted` faux partout) et **47 groupes de constats restent ouverts** ; l'élévation latérale debout qui sert aussi aux variantes assises/inclinées est désormais tracée comme les autres.
- **Registres** : `candidate/validation.json` (blocs `apkDeliveryDryRun` puis `apkDeliveryRelease`), `review/findings.json` (47), `review/never-reviewed.json`, `review/exercise-coverage.json`, `review/REVIEW-POOL.md`, `REVIEW-ALIASES.md`, `REVIEW-ILLUSTRATIONS.md`, `REVIEW-APK-DRYRUN.md`, `REVIEW-WARMUP.md`.

**Contrôle du lien lui-même** : l'APK a été **retéléchargé depuis GitHub après publication** (API des contenus, commit `f69f469`) puis comparé octet pour octet au candidat signé : mêmes 24 909 281 octets, même SHA-256, et `apksigner` y retrouve *Verifies*, v2 et v3 vrais, un signataire, certificat `d3ca5a27…`. Le lien ne sert donc pas un fichier différent de celui testé.

### Prochaine étape précise

Test physique par l'utilisateur : exporter le JSON, désinstaller la 1.4.0, installer la 1.4.1, restaurer, puis vérifier sur le téléphone les blocs **piscine après musculation** et la conservation des charges/historique. Ensuite reprendre l'audit des visuels : décider avec l'utilisateur la **règle de style** avant toute nouvelle illustration, traiter les **47 groupes ouverts** sans en fermer un seul silencieusement, en priorité les écarts de matériel (curl debout pour banc Scott, mollets debout pour presse à mollets, développé debout pour nuque/un bras, relevé suspendu pour relevé incliné, élévation latérale debout pour variantes assises/inclinées), puis les approches d'échauffement restantes et l'affichage hérité des durées en secondes dans la modale. Maintenir les bonnes animations, la récupération aquatique, les 209 exercices, les 11 rubriques, les deux profils, les étapes 1–7 et l'accueil validé. **IA conversationnelle en dernier ; actualiser ET présenter cette passation à chaque étape, ici dans le même chat.** Aucun changement de chat imposé ; avertir avec 🚩 avant la limite.
