...
**Mise à jour : 22 septembre 2026.** Lire ce document avant de poursuivre dans une nouvelle conversation. Les fichiers et commits accessibles sont la source de vérité ; un ancien message annonçant un fichier ne garantit pas sa présence actuelle.

## 🚩 Reprise — correctifs des visuels construits, APK 1.5.0 à signer

**Branche de travail : `arena/01a0c424-jarvis-fitness-yanis-emilie-ap`** (poussée après chaque étape ; le bac à sable a déjà été réinitialisé une fois, tout travail non poussé est perdu). Ne pas travailler sur `main` directement, pas de reset destructif. IA conversationnelle toujours en pause.

### État vérifié au 22 septembre 2026

- **Les correctifs des démonstrations sont intégrés dans un bundle 1.5.0** construit à partir de l’exact bundle 1.4.0 : `node evolution/media/build.mjs` → `.cache/media-web/`. Rapport complet : **`evolution/media/README.md`** (section « Ce qui est livré »). Table de revue : `evolution/media/mapping.json` (209 exercices : 119 exact / 64 variante annotée / 26 « Pas de démonstration » ; 42 mouvements Tabata au sol ; 19 guides piscine ; 29 étirements ; échauffement). Aperçu avant/après : `evolution/media/review/corrections-apercu.jpg`.
- **Dips** : image haute redessinée dans le même style (`evolution/media/assets/dips-triceps-corrige.gif`, 2 positions fixes) ; l’original reste dans l’APK, choix réversible (`mapping.dips.variant`). La proposition photographique `dips-comparaison.jpg` n’est **pas** utilisée.
- **Tests** : 11 tests Node (`node --test evolution/media/tests/*.test.mjs`), **118 contrôles de rendu réel** dans happy-dom (`node evolution/media/tests/dom-smoke.mjs`, après `cd .cache/dom-tools && npm install happy-dom`) : accueil, chrono Tabata sol/aqua/échauffement/étirements, fiches (Yanis et Émilie), protocoles piscine, liste d’étirements ; 5 tests Python du repack (`python3 -m unittest evolution/android/tests/test_media_release.py`). **Playwright/Chromium n’est pas téléchargeable dans ce bac à sable** ; les specs Playwright historiques n’ont pas été relancées ici.
- **Android** : `python3 evolution/android/build-media.py --unsigned-candidate` produit et vérifie un candidat **non signé** (`.cache/media-release/unsigned-candidate.apk`) : 1.4.0 → **1.5.0 / code 12**, package `app.yanis.fitness.evolution.home` inchangé, 9 DEX + `resources.arsc` + `capacitor.config.json` identiques, seuls le manifeste (version) et le bundle changent, 1 média ajouté (`assets/public/media/dips-triceps-corrige.gif`). Recette : `evolution/android/release-media.json` (pin du bundle validé `validatedMediaBundleSha256`).
- **Bloqué ici : la signature.** `.private/yanis-fitness-evolution-home/` (sauvegarde 1.4.0 restaurée : `recovery-key.txt` + ZIP privé), le JDK `.cache/signing-tools/jdk4py` et `apksigner` (`.cache/home-tools/apksigner.jar`, SHA `ef494179…`) sont absents de cet environnement. **Ne pas générer de clé, ne pas créer d’identité, ne pas demander de secret dans le chat.** Sur la machine qui possède la sauvegarde privée : `python3 evolution/android/build-media.py` signe avec la même clé (certificat `7d6f9c8f…21fd`) et écrit `downloads/Yanis-Fitness-Evolution-1.5.0.apk` + `.fidelity.json` + `.sha256` ; puis rédiger `downloads/INSTALLATION-1.5.0.md` (mise à jour par-dessus la 1.4.0, données conservées, pas de nouvelle application) et donner le lien après vérification du fichier publié.
- Aperçu navigateur de la 1.5.0 : `python3 -m http.server 8080 --bind 0.0.0.0` dans `.cache/media-web/` (état local, deux profils). Ne jamais servir la racine du dépôt ni `.private`.

### Lacunes assumées (visibles dans l’application, pas masquées)
...
### Lot 1 intégré (22/09, après « Poursuis ») — lot 2 en attente

- **Lot 1 intégré et réversible** : jumping jacks, burpees (+ simplifiés en variante), squats sautés, montées de genoux (+ high knees, marche sur place en variante), patineurs, chaise au mur (+ chaise douce), superman, russian twist → GIF dans `evolution/media/assets/`, noms dans `VALIDATED_PROPOSALS` (`review/compose-mapping.py`). Retirer un nom = retour à la carte d’absence. Recette Android : 9 entrées média, pin `validatedMediaBundleSha256` renouvelé, candidat non signé revérifié, 127 contrôles DOM.
- **Lot 2 proposé, NON intégré** (`review/proposals-lot2.jpg`, GIF dans `assets/proposals/`) : dips au bord, squats sumo, battements de jambes (sol), crunch, corde invisible, mobilité hanches & chevilles (échauffement : à router vers la table `warmup` et le patch `Bg` lors de l’intégration).
- **À refaire (limite de génération d’images atteinte dans le tour)** : activation scapulaire (position 2 = rotation externe coudes au corps), étirements « Mollet en escalier » (talon sous la marche) et « Adduction de la hanche debout » (jambes croisées) — style des fiches d’étirement (`stretch-*.jpg`, une seule position).
- Environnement réinitialisé une deuxième fois le 22/09 : tout est sur la branche distante ; remise en état = `git merge --ff-only origin/arena/01a0c424-…`, `npm install --no-save acorn` dans `JARVIS-Fitness-Source`, `pip install --target .cache/image-tools pillow`, `npm install happy-dom` dans `.cache/dom-tools`, extraction du web 1.4.0 vers `.cache/media-audit/web-1.4.0/`.

### Historique : propositions lot 1 (texte d’origine)
...
2. Si la sauvegarde privée est disponible : signer la 1.5.0 avec `build-media.py`, vérifier `test_media_release.py` avec `MEDIA_APK=downloads/Yanis-Fitness-Evolution-1.5.0.apk`, publier, notice d’installation, lien direct.
3. Sinon : proposer dans le chat les médias pour les lacunes (par lots, mêmes critères matériel/position/contexte), sans toucher aux exercices ni aux consignes.
4. Toute modification du bundle exige de renouveler le pin `validatedMediaBundleSha256` et de rejouer `dom-smoke.mjs`.
...
- Privé : `.private/yanis-fitness-evolution-home/` (hors Git, jamais servi par HTTP) — présent lors de la fabrication 1.4.0, **absent du bac à sable du 22 septembre 2026** ; la signature de la 1.5.0 exige sa restauration depuis le ZIP privé conservé par l’utilisateur.
...
> Reprends Yanis Fitness Evolution. Lis d’abord `PASSATION.md` et `evolution/media/README.md` dans le dépôt `Yandevil974/JARVIS-Fitness-Yanis-Emilie.apk`, branche de travaux `arena/01a0c424-jarvis-fitness-yanis-emilie-ap`, en respectant la branche imposée à ta session. Les correctifs des visuels (musculation, échauffement, piscine, Tabata sol/aqua, étirements) sont construits dans un bundle 1.5.0 (`evolution/media/build.mjs`, table `mapping.json`, tests Node + rendu DOM, repack Android `evolution/android/build-media.py` vérifié en candidat non signé). Il reste : signer la 1.5.0 avec la MÊME clé et le MÊME package `app.yanis.fitness.evolution.home` que la 1.4.0 (sauvegarde privée requise, absente du dernier bac à sable — ne jamais générer de clé ni demander un secret), publier le lien et la notice, puis proposer dans le chat des médias pour les lacunes listées (26 exercices, 15 mouvements Tabata au sol, 2 étirements, 2 étapes d’échauffement) sans modifier les exercices ni leurs consignes. Conserve les visuels d’origine corrects ; aucun remplacement global par des photos n’est validé. Préserve les deux profils, les données, les sept étapes et l’accueil validé. L’IA reste en pause. Actualise la passation avant de poursuivre trop loin.
