    ...
    <aside className={`sidebar ${menu ? "open" : ""}`}>
      ...
      <div className="sidebar-bottom">
        ...
        <div className="system-status">
          <i /> JARVIS SYSTEM ONLINE <span>V {APP_VERSION}</span>
        </div>
      </div>
    </aside>
  );
    ...
    const check = () => {
      ...
      for (const s of p.plan?.sessions || []) {
        ...
        if (
          ...
          sendSystemNotification("JARVIS · Votre séance", text, {
            tag: key,
      ...
      const reval = forceRevalState(p);
      if (reval.due) {
        const key = `force-reval-${reval.next || "initial"}`;
        if (!p.notifications.some((n) => n.key === key)) {
          const text = reval.done
            ? `${reval.label} Vos charges sont calculées sur un bilan du ${reval.last}. Refaites le test pour qu’elles restent justes.`
          ...
          updateProfile((q) => {
            if (q.notifications.some((n) => n.key === key)) return;
            q.notifications.unshift({
              id: uid(),
              key,
              date: today(),
          ...
          sendSystemNotification("JARVIS · Bilan de force", text, {
            tag: key,
            enabled: p.preferences.notifications,
          });
        }
