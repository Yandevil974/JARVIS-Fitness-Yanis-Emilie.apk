...
export async function loadState() {
  ...
  // Reprise de lecture (jamais d'écriture) des données JARVIS existantes si
  // cette application cohabite sur le même stockage web : chaque candidate est
  // validée par le schéma avant usage, l'ancienne clé n'est jamais modifiée.
  if (!candidates.length)
    for (const legacyKey of LEGACY_STORAGE_KEYS)
      try {
        ...
        if (raw)
          candidates.push({
            source: "Import JARVIS (lecture seule)",
            data: safeJSON(raw),
          });
      } catch (e) {}
