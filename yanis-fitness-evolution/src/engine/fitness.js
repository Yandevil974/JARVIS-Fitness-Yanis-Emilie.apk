...
export function recommendLoad(p, target, date = today()) {
  ...
  if (!sets.length) {
    // 1.5.0 : la première charge vient du pipeline de référence complet
    // (1RM déclaré ou recalculé, ratio de famille, règle de progression,
    // plafond 68 % en semaine allégée), pas d’un simple pourcentage fixe.
    const strength = loadForExercise(p, ex, target.repScheme || `${low}-${high}`, {
      deload: !!target.deload || (rec.score != null && rec.score < 45),
      unit,
    });
    if (strength?.load != null) {
      const capped =
        rec.score != null && rec.score < 50
          ? roundLoad(strength.load * 0.9, increment, "down")
          : strength.load;
      return {
        ...base,
        weight: capped,
        reps: Math.max(low, Math.min(high, parseTargetReps(target.repScheme) || low)),
        decision: "keep",
        color: strength.fromJournal ? "blue" : "amber",
        label: strength.fromJournal
          ? "Charge calculée · vos séances"
          : "Charge calculée · bilan 1RM",
        reason:
          forceSourceLabel(strength) +
          (capped !== strength.load
            ? " Récupération faible aujourd’hui : −10 % appliqués."
            : ""),
        reference: strength.lastDate || strength.declaredDate || null,
        strength,
      };
    }
    return base;
...
// Lecture des photos de progression (port de la 1.5.0) : compare les deux
// séries datées et chiffre l'évolution avec les mensurations communes.
export function photoReading(profile) {
  const n = (profile.photos || [])
    .filter((m) => m.date)
    .sort((a, b) => a.date.localeCompare(b.date));
  if (n.length < 2)
    return {
  ...
  const first = n[0].date,
    last = n[n.length - 1].date,
    near = (d) =>
      [...(profile.measurements || [])]
        .filter((x) => x.date && x.date <= d)
  ...
  if (a?.weight != null && b?.weight != null) {
    ...
    bits.push(
      d === 0 ? "poids stable" : `${d > 0 ? "+" : ""}${d} kg (${a.weight} → ${b.weight})`,
    );
  }
  ...
  return {
    ...
    fin: last,
    text: bits.length
      ? `Entre le ${first} et le ${last} : ${bits.join(", ")}. Comparez vos photos côte à côte dans Progression : à poids voisin, un changement visible signale une recomposition, ce que la balance seule ne montre pas.`
      : `Photos du ${first} et du ${last} disponibles. Aucune mensuration commune aux deux dates : renseignez poids et tour de taille pour que la comparaison soit chiffrée.`,
  };
}
