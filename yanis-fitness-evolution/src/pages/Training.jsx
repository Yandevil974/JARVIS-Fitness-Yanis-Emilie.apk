...
import { loadForExercise, forceSourceLabel } from "../engine/force.js";
import {
  EXERCISES,
  exerciseById,
  GROUPS,
...
function Workout() {
  ...
  // Charge conseillée par le moteur de référence 1RM (1.5.0) : sert à la fois
  // de pré-remplissage du formulaire et de suggestion affichée dans le rail.
  const schemeReps =
    target.repTargets?.[done % (target.repTargets?.length || 1)] ||
    target.repsLow;
  const strengthLoad = loadForExercise(
    p,
    ex,
    schemeReps ?? target.repScheme,
    { deload: !!target.deload, unit: target.unit },
  );
  ...
  useEffect(() => {
    setWeight(target.targetLoad ?? strengthLoad?.load ?? "");
    setReps(
      ex.timed
        ? target.seconds || 30
      ...
      <div className="immersive-layout">
        ...
        <aside className="session-rail">
          ...
          <Panel className="session-recommendation">
            ...
            {strengthLoad?.load != null && (
              <div className="load-suggestion">
                <div className="load-suggestion-value">
                  <strong>{strengthLoad.load}</strong>
                  <span>
                    {ex.unit === "kg/main" ? "kg par haltère" : "kg"} ·{" "}
                    {schemeReps} rép.
                  </span>
                </div>
                <p>{forceSourceLabel(strengthLoad)}</p>
                {strengthLoad.effective != null && (
                  <small>
                    Base : {strengthLoad.baseName} {strengthLoad.effective} kg à
                    1RM
                    {strengthLoad.percent
                      ? ` · ${strengthLoad.percent} % du maximum`
                      : ""}
                    {strengthLoad.fromJournal
                      ? " · estimé depuis vos séries"
                      : strengthLoad.declared
                        ? " · issu de votre bilan 1RM"
                        : ""}
                  </small>
                )}
                {Number(weight) !== strengthLoad.load && (
                  <button
                    className="text-link"
                    onClick={() => setWeight(strengthLoad.load)}
                  >
