...
export function WarmupModal({ session }) {
  const { p, setTimer, updateProfile, notify, closeModal, setModal } =
    useApp();
...
// Retour au calme ciblé sur les muscles réellement travaillés dans la séance
// portée (port exact du comportement 1.5.0).
export function CooldownModal({ session }) {
  ...
  for (const row of s?.exercises || []) {
    ...
    if (ex) {
      ...
      for (const sec of ex.secondary || []) muscles.add(sec);
    }
  }
  const all = RECOVERY_EXERCISES.filter((e) => e.pattern === "stretch");
  const targeted = all.filter((e) => muscles.has(e.muscle));
  const chosen = (targeted.length ? targeted : all).slice(0, 8);
  const total = chosen.reduce((n, e) => n + (e.seconds || 30), 0);
  return (
    <Modal
      title="Terminer en douceur."
      subtitle={`Retour au calme · ${chosen.length} étirements ciblés sur les muscles travaillés, environ ${Math.round(total / 60)} minutes.`}
      onClose={closeModal}
      wide
    >
      <div className="warmup-detail">
        {chosen.map((ex, i) => (
          <div key={ex.id}>
            ...
            <Badge>{durationLabel(ex.seconds || 30)}</Badge>
          </div>
        ))}
      </div>
      <div className="modal-actions">
        ...
        <Button
          variant="secondary"
          icon="BookOpen"
          onClick={() => setModal({ type: "stretch", exercise: chosen[0] })}
        >
          Voir en détail
        </Button>
...
// Visionneuse : agrandit une image d’étape, ou agrandit l’animation créée.
export function ImageViewer({ src, title, motion }) {
  const { closeModal } = useApp();
  if (!src && !motion) return null;
  return (
    <Modal title={title || "Mouvement"} onClose={closeModal} wide>
      <div className="image-viewer">
        {src ? (
          ...
          <HumanAnim spec={motion} className="zoomed" />
        )}
      </div>
      <div className="modal-actions">
