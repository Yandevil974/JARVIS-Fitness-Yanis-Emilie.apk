...
function resolvePoolPattern(name) {
  const n = norm(name);
  if (/aqua.jogging/.test(n)) return "aquaJog";
  if (/montee.*genou/.test(n)) return "kneeRaise";
  if (/ciseaux/.test(n)) return "scissor";
  if (/deplacement.*lateral/.test(n)) return "lateral";
  if (/talon.*fesse/.test(n)) return "buttKick";
  if (/pompe.*bord/.test(n)) return "poolPushup";
  if (/gainage.*bord|gainage.*vertical/.test(n)) return "plankVertical";
  if (/battement/.test(n)) return "flutterKick";
  if (/marche aquatique/.test(n)) return "aquaWalk";
  if (/nage statique/.test(n)) return "staticSwim";
  if (/nage douce/.test(n)) return "gentleSwim";
  if (/fractionne|sprint/.test(n) && /nager/.test(n)) return "swimSprint";
  if (/sprint.*nager|nager.*sprint/.test(n)) return "swimSprint";
  if (/souffler|recup complete/.test(n)) return "aquaRest";
  if (/recup entre tabata/.test(n)) return "aquaRest";
  if (/retour au calme/.test(n)) return "gentleSwim";
  if (/etirement.*bord/.test(n)) return "stretchPool";
  if (/mobilite.*epaule/.test(n)) return "shoulderMobility";
  if (/mobilite.*hanche|mobilite.*cheville/.test(n)) return "hipMobility";
  if (/repos|respiration|recup/.test(n)) return n.includes("marche") ? "aquaWalk" : "aquaRest";
  if (/echauffement.*battement/.test(n)) return "flutterKick";
  if (/echauffement.*marche/.test(n)) return "aquaWalk";
  if (/echauffement.*nage/.test(n)) return "gentleSwim";
  return "swim";
}
...
export function TimerModal() {
  ...
  return (
    ...
    >
      <div className={`timer-stage ${step.kind || ""} pattern-${step.pattern || "unknown"}`}>
        <Badge
  ...
  function launch() {
    ...
    setTimer(
      lv.steps.map(([name, seconds]) => {
        const isRest = /repos|respiration|recup|retour au calme/.test(norm(name));
        const pattern = isRest
          ? norm(name).includes("marche")
            ? "aquaWalk"
            : "aquaRest"
          : resolvePoolPattern(name);
        return {
          name,
          seconds,
          pattern,
          kind: isRest ? "rest" : "work",
          segment: isRest ? "rest" : "work",
        };
      }),
      {
  ...
  return (
    ...
    >
      ...
      <div className="warmup-visual">
        <Movement pattern={steps[1]?.pattern || "walk"} small />
        <div>
    ...
    >
      ...
      <div className="info-line">
        ...
        <span>
          ...
          vectorielle humaine montre la position cible : suivez en priorité les consignes de
          position.
        </span>
      </div>
      <div className="modal-actions">
