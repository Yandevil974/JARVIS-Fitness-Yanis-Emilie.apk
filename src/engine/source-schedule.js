...
function resolvePoolPatternForStep(name) {
  ...
  if (n.includes("fractionne") || n.includes("sprint")) return "swimSprint";
  if (n.includes("souffler") || n.includes("recup complete")) return "aquaRest";
  if (n.includes("recup entre tabata")) return "aquaRest";
  if (n.includes("retour au calme")) return "gentleSwim";
  if (n.includes("etirement")) return "stretchPool";
  if (n.includes("mobilite") && n.includes("epaule")) return "shoulderMobility";
  if (n.includes("mobilite") && (n.includes("hanche") || n.includes("cheville"))) return "hipMobility";
  if (n.includes("repos") || n.includes("recup") || n.includes("respiration")) return "aquaRest";
  return "swim";
}
    ...
    if (part.key === "pool") {
      ...
      steps.push(
        ...sourcePool(p, part.protocolId, part.level).steps.map(
          ([name, seconds]) => {
            const isRest = /repos|récup|calme|respiration|souffler/i.test(name);
            const pattern = isRest
              ? /marche/i.test(name)
                ? "aquaWalk"
                : "aquaRest"
              : resolvePoolPatternForStep(name);
            return {
              name,
