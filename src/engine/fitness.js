...
export function recommendLoad(p, target, date = today()) {
  ...
  if (!sets.length) {
    const test = p.forceTests
      .filter(
        (t) => t.exerciseId === ex.id && t.unit === unit && t.estimate > 0,
      )
      .sort((a, b) => a.date.localeCompare(b.date))
      .at(-1);
    if (test) {
      const intensity = p.user.goal === "strength" ? 0.8 : 0.7;
      return {
        ...base,
        weight: roundLoad(
          test.estimate *
            intensity *
            (rec.score != null && rec.score < 50 ? 0.9 : 1),
          increment,
          "down",
        ),
        decision: "keep",
        color: "amber",
        label: "Référence 1RM",
        reason: `Proposition prudente à ${Math.round(intensity * 100)} % du 1RM de cet exercice (${test.estimate} kg). Ajustez à votre ressenti.`,
        reference: test.date,
      };
    }
    return base;
...
export function warmup(p, session) {
  ...
  // Coherent human animations for each warmup phase
  return [
    {
      name: "Mise en route",
      seconds: 180,
      ...
      segment: "warmup",
      instruction:
        "Marche ou vélo très facile. Vous devez pouvoir parler confortablement. Animation : marche humaine douce.",
    },
    {
      name: lower ? "Mobilité hanches & chevilles" : "Mobilité des épaules",
      seconds: 60,
      pattern: lower ? "hipMobility" : "shoulderMobility",
      kind: "warmup",
      segment: "mobility",
      instruction: lower
        ? "Mobilisez doucement les chevilles et les hanches, sans forcer. Animation : balancement de jambe et cercles de chevilles."
        : "Cercles d’épaules lents, puis mouvements scapulaires contrôlés. Animation : cercles de bras humains.",
    },
    {
      ...
      segment: "activation",
      instruction: lower
        ? "Ponts fessiers au sol, 10 répétitions contrôlées. Animation : pont fessier humain."
        : "Rétractions des omoplates et rotations externes sans fatigue. Animation : rowing léger humain.",
    },
    ...[0.5, 0.7, 0.85].map((pct, i) => ({
      ...
      kind: "warmup",
      segment: "approach",
      instruction: `${[10, 6, 3][i]} répétitions faciles${w > 0 ? " à environ " + Math.round(pct * 100) + " % de la charge de travail connue" : ". Donnée insuffisante pour une charge chiffrée"}. Repos selon le besoin. Animation adaptée à ${ex?.name || "l'exercice principal"}.`,
    })),
  ];
}
