...
function Hiit() {
  ...
  function start() {
    ...
    function resolveAquaPatternLocal(name) {
    ...
    for (let c = 0; c < cycles; c++) {
      for (let r = 0; r < rounds; r++) {
        const moveName = movements[r % movements.length];
        steps.push({
          name: `${moveName} · round ${r + 1}/${rounds}`,
          seconds: work,
          pattern: aqua
            ? resolveAquaPatternLocal(moveName)
      ...
      if (c < cycles - 1)
        steps.push({
          name: aqua ? "Récup entre tabatas — marche douce" : "Récupération entre cycles",
          seconds: 120,
          pattern: aqua ? "aquaRest" : "breathe",
          kind: "rest",
          segment: "transition",
